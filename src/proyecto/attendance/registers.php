...
if (!isset($_GET["curse_id"])) {
    header("Location: ../main.php");
}
include_once "attendance_function.php";
$curse = getCurseByID($_GET["curse_id"]);

?>
<div class="row" id="app">
    <div class="col-12">
        <h1 class="text-center" style="color:red;">Asistencias en '<?php echo $curse->name ?>'</h1>
    </div>
    <div class="col-12">
        <div class="form-inline mb-2">
            <label for="date">Día: &nbsp;</label>
            <input @change="refreshEmployeesList" v-model="date" name="date" id="date" type="date" class="form-control">
            <button @click="save" class="btn btn-success ml-2">Guardar <i class="fa fa-check"></i></button>
        </div>
    </div>
    <div class="col-12">
                    ...
                    <tr v-for="employee in employees">
                        ...
                        <td>
                            <select v-model="employee.status" class="form-control">
                                ...
                                <option value="absence">No presente</option>
                            </select>
                        </td>
                    </tr>
                </tbody>
    ...
    new Vue({
        ...
        methods: {
            ...
            async refreshEmployeesList() {
                // Get all employees
                let response = await fetch(`./get_students_ajax.php?curse=${this.curse}`);
                ...
                response = await fetch(`./get_attendance_data_ajax.php?date=${this.date}&curse=${this.curse}`);
